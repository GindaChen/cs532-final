...
X = X .* (1 ./ max(X)); % Regularize
% If you experience error, in the above sentence, try this instead
% X = [ sin(x * (1./ t))   cos(x * (1./ t)) one_column  x./length(x)]; % 
...
X = X .* (1 ./ max(X));
% Or alternatively:
% X = [ ones(size(x))  x./length(x) sin(x * (1./ t))   cos(x * (1./ t)) ]; % 
